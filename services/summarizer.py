...
async def clean_transcript(transcript: str) -> str:
    """
    Fixes phonetic transcription errors in technical terms, brand names, and proper nouns.
    Uses gpt-4o-mini — fast and cheap. Only corrects clear errors, never rephrases.
    """
    prompt = (
        "You are a transcript corrector for an Indian tech startup called Ladder.\n"
        "The transcript was generated by speech-to-text AI from Hindi/Hinglish speech.\n"
        "Your ONLY job: fix phonetic transcription errors for technical terms, brand names, "
        "product names, tool names, company names, and proper nouns.\n\n"
        "Examples of the kind of errors to fix:\n"
        "- 'clawbot' or 'claw bot' → 'Claude bot'\n"
        "- 'click up' → 'ClickUp'\n"
        "- 'recall dot AI' → 'Recall.ai'\n"
        "- 'eleven labs' → 'ElevenLabs'\n"
        "- 'deep ram' → 'DeepRAM'\n"
        "- '12 labs' → '12Labs'\n"
        "- Garbled Indian names, company names, tool names\n\n"
        "STRICT RULES:\n"
        "- Only fix clear phonetic errors for proper nouns / tech terms\n"
        "- Do NOT rephrase, summarize, shorten, or change sentence structure\n"
        "- Do NOT fix grammar or filler words\n"
        "- Return the FULL corrected transcript — same length, same structure\n\n"
        f"TRANSCRIPT:\n{transcript}"
    )
    try:
        resp = await client.chat.completions.create(
            ...
            max_completion_tokens=len(transcript.split()) * 2,
            timeout=60
        )
        cleaned = resp.choices[0].message.content.strip()
        print(f"[Transcript] Cleaned ({len(transcript)} → {len(cleaned)} chars)")
        return cleaned
    except Exception as e:
        print(f"[Transcript] Clean failed ({e}) — using raw transcript")
        return transcript
